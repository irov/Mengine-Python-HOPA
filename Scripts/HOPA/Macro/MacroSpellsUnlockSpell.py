...
class MacroSpellsUnlockSpell(MacroCommand):
    def _onValues(self, values):
        self.spell_type = values[0]

        self.notificator = Notificator.onSpellUISpellUnlock

    def _onInitialize(self):
        if _DEVELOPMENT is True:
            if SpellsManager.getSpellsUIButtonParam(self.spell_type) is None:
                self.initializeFailed("MacroSpellsUnlockSpell SpellsManager: not found spell type '{}'".format(self.spell_type))
                pass
            pass
