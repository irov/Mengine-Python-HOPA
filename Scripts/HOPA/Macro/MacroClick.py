...
class MacroClick(MacroCommand):
    ...
    def _onGenerate(self, source):
        ...
        if isRepeat is False or self.CreateQuestOnRepeat is True:
            Quest = self.addQuest(source, "Click", SceneName=self.SceneName, GroupName=self.GroupName, Object=self.Object)

            with Quest as tc_quest:
                tc_quest.addTask("AliasObjectClick", SceneName=self.SceneName, Object=self.Object, AutoEnable=self.AutoEnable)
        else:
            source.addTask("AliasObjectClick", SceneName=self.SceneName, Object=self.Object, AutoEnable=self.AutoEnable)

        source.addNotify(Notificator.onMacroClick)