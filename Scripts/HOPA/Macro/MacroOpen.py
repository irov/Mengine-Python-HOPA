    ...
    def _onInitialize(self):
        if _DEVELOPMENT is True:
            ...
            if ObjectType not in self.ObjectsTypeFilter:
                self.initializeFailed("MacroOpen: %s:%s invalid type" % (ObjectType, self.ObjectName))
                pass
            pass
        pass

    def _onGenerate(self, source):
        source.addTask("TaskSetParam", ObjectName=self.ObjectName, Param="BlockOpen", Value=False)
        pass
