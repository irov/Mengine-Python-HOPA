...
class MacroSpellAmuletRemovePower(MacroCommand):
    def _onValues(self, values):
        ...
        self.notificator = Notificator.onSpellAmuletRemovePower

    def _onInitialize(self):
        if _DEVELOPMENT is True:
            if SpellsManager.getSpellAmuletStoneParam(self.power_type) is None:
                self.initializeFailed("MacroSpellAmuletRemovePower SpellsManager: not found power type '{}'".format(self.power_name))
                pass
            pass
    ...
    def _onGenerate(self, source):
        ...
        with source.addParallelTask(2) as (parallel_0, parallel_1):
            ...
            parallel_1.addNotify(self.notificator, self.power_type)
